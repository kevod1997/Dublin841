...
const DayTurnsComponent = () => {
  ...
  const {pickDay, setPickDay, turns, setTurns} = useAdmin()
    ...
    <table className="table-fixed w-full mx-auto">
      <thead>
        <tr>
          ...
          <th className="w-1.5/6 px-2 sm:px-4 py-1 text-sm">Teléfono</th>
          <th className="w-1/6 px-2 sm:px-4 py-1 text-sm">Corte</th>
          <th className="w-1/6 px-2 sm:px-4 py-1 text-sm"></th>
        </tr>
      </thead>
      <tbody>
        {sortedTurns.map((turn, index) => (
          <tr key={index}>
            ...
            <td className="border px-2 sm:px-4 py-1 text-sm">
